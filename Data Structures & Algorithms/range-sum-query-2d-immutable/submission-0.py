class NumMatrix:

    def __init__(self, matrix: List[List[int]]):
        m = len(matrix)
        n = len(matrix[0])
        # Init extra row and column of zeros
        self.matrixSum = [[0] * (n + 1) for _ in range(m + 1)]

        for i in range(m):
            prefix = 0
            for j in range(n):
                prefix += matrix[i][j]
                # row above
                above = self.matrixSum[i][j + 1]
                self.matrixSum[i + 1][j + 1] = prefix + above

    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        r1, c1, r2, c2 = row1+1, col1+1, row2+1, col2+1

        bottomRight = self.matrixSum[r2][c2]
        top = self.matrixSum[r1 - 1][c2]
        left = self.matrixSum[r2][c1 - 1]
        topLeft = self.matrixSum[r1-1][c1-1]

        # Add topLeft because its subtracted twice
        return bottomRight - top - left + topLeft        


# Your NumMatrix object will be instantiated and called as such:
# obj = NumMatrix(matrix)
# param_1 = obj.sumRegion(row1,col1,row2,col2)